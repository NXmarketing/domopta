...
$(document).ready(function(){
    ...
    for (let i = 0; i < inputs.length; i++) {
        if (inputs[i].value) {
            ...
            inputs[i].parentNode.querySelector('.active-input').style.display = "block";
            }
        }
    }
    document.querySelector('.wrapper').addEventListener('input', function(e){
        if (e.target.value) {
            ...
            e.target.parentNode.querySelector('.active-input').style.display = "block";
        }
    });
    //--------------------------------------------------------- header-top
    ...
    var sliderW = $('.products__list_w').lightSlider({
        ...
        slideMargin: 0,
        loop:true,
        prevHtml: '<div class="products__arrow products__arrow_l"><span class="products__link products__icon"><svg class="svg products__svg products__svg_arrow2-left"><use xlink:href="/img/sprite-sheet.svg#arrow2-left"/></svg></span></div>',
        nextHtml: '<div class="products__arrow products__arrow_r"><span class="products__link products__icon"><svg class="svg products__svg products__svg_arrow2-right"><use xlink:href="/img/sprite-sheet.svg#arrow2-right"/></svg></span></div>',
        responsive: [
            {
                breakpoint: 1200,
                settings: {
...
//    },500)
    ...
    function init(){ 
        ...
        var placemark = new ymaps.Placemark([44.93335454, 34.11270504],{}, {
			iconColor: '#ff0000'
        });
...
if (window.innerWidth < 1200) {
    $('.common__heading').click(function(e){
        e.preventDefault();
        e.target.closest('.common').querySelector('.common__list').classList.toggle('common__list_show');
    });
}
    ...
    $('#cat').click(function(e){
        e.preventDefault();
        e.target.closest('.drop-content').classList.toggle('drop-content_show');
        document.querySelector('.category__list').classList.toggle('category__list_show');
    });
    $('#cab').click(function(e){
        e.preventDefault();
        e.target.closest('.drop-content').classList.toggle('drop-content_show');
        document.querySelector('.user-btns__list').classList.toggle('user-btns__list_show');
    });
...
//                }
    ...
    $('body').on('click', '.product__icon-heart, .tag-tovar-btn__link', function (e) {
        e.preventDefault();
        if($('#enter').length == 0) {
            $.get('/favorites/add', {'product_id': $(this).data('id')}, function (response) {
